...
#define		RS232_BUFFER_SIZE			8192
#define		SOCKET_TIMEOUT_VAL			20000
#define		RESPONSE_TIMEOUT_VAL		5



#define		LED_R232_SHOT			"echo 1 > /sys/devices/platform/leds/leds/LED:R232/shot"
#define		LED_G232_SHOT			"echo 1 > /sys/devices/platform/leds/leds/LED:G232/shot"
...
result rs232_SerialPort_Open(SerialPort *SerialPORT)
{
	...
	if (SerialPORT->Port_fd < 0) {
		...
		report("ss","RS232 --> Error file open:", strerror(errno));
	}
	...
	if (tcsetattr(SerialPORT->Port_fd , TCSANOW, &tty) != 0) {
		report("ss","RS232 --> PORT_OPEN_ERROR= ", strerror(errno));
	  return ERROR;
	}
	else
	{
		report("s","RS232 --> PORT_OPEN_SUCCESSFULL!");
		return OK;
	}
...
{
	if(SerialPORT->Aes.Enable==true) rs232_AES_Decrypt(&SerialPORT->Aes,SerialPORT->TX_Buffer,SerialPORT->TX_Count);
...
void *rs232_TCPClient_ConnectionCheck(void *arg)
{
	...
	while(1)
	{
		for(i=0;i<MAX_SERVER_NUMBER;i++)
		{
			...
			if(ClientSocket.Error!=0) report("sdsd","RS232 --> TCP-Client:Socket",i,"--> Connection Error =",ClientSocket.Error);
		}
...
void *rs232_TCPServer_ConnectionCheck(void *arg)
{
	...
	report("s","RS232 --> TCP_SERVER_CONNECTION_CHECK_Thread Started !");
	...
	while(1)
	{
		...
		if(r1==1)	exec2(LED_STATUS_OFFDLY3);
		else        exec2(LED_STATUS_OFFDLY1);
		...
		{
			report("s","RS232 --> TCP-Server --> Connection Check select error !");
		}

		//Check for New Connection To this Server
		if (FD_ISSET(ServerSocket.Server_fd, &readfds))
		{
			report("s","RS232 --> TCP-Server --> New Connection !!");
...
void *rs232_TCPClient_Receive(void *arg)
{
	...
	int num_bytes=0;
	uint8_t Buffer[RS232_BUFFER_SIZE],i;
	...
	report("s","RS232 --> TCP_CLIENT_RX_Thread Started !");
	...
	while(1)
	{
		...
		if(activity==0)
		{
			 if(ClientSocket.ReceiveFlag >-1)
			 {
				 ...
				 {
					 report("sdsd","RS232 -->  TCP-Client:Socket",ClientSocket.ReceiveFlag,"  ->>- PORT --  Count=",SerialPORT.TX_Count);
					 rs232_SerialPort_Send(&SerialPORT);
				 }
		...
		else if( activity>0)
		{
			...
			for(i=0 ; i < MAX_SERVER_NUMBER ; i++)
			{
				...
				{
					num_bytes = read( ClientSocket.Server_fd[i] , Buffer, RS232_BUFFER_SIZE);
					if(num_bytes == 0)
					{
						report("sds","RS232 --> TCP-Client:Socket",i,"--> Connection Error !");
					...
					{
						report("sdsd","RS232 --> TCP-Client:Socket",i," --> ReadFD error =",errno);
						ClientSocket.Status[i].Connected=false;
					}
				}
			}
...
void *rs232_TCPServer_Receive(void *arg)
{
	...
	uint8_t Buffer[RS232_BUFFER_SIZE],i=0;
	...
	report("s","RS232 --> TCP_SERVER_RX_Thread Started !");
	...
	while(1)
	{
		...
		if(activity==0)
		{
			//printf("activity=%d\n",activity);
			 if(ServerSocket.ReceiveFlag >-1)
			 {
				 if(SerialPORT.TX_Count>0)
				 {
					 report("sdsd","RS232 -->  TCP-Server:Socket",ServerSocket.ReceiveFlag,"  ->>-  PORT --  Count=",SerialPORT.TX_Count);
				 	 rs232_SerialPort_Send(&SerialPORT);
				 }
		...
		else if(activity>0)
		{
			...
			for(i=0 ; i < MAX_CLIENT_NUMBER ; i++)
			{
				...
				{
					num_bytes = read( ServerSocket.Client_fd[i] , Buffer, RS232_BUFFER_SIZE);
					if(num_bytes == 0)
					{
						getpeername(ServerSocket.Client_fd[i] , (struct sockaddr*)&Serv_addr.sin_addr,(socklen_t*)&AddLen);
						report("sssd","RS232 --> TCP-Server -> Host disconnected: ip=",inet_ntoa(Serv_addr.sin_addr)," , port=",ntohs(Serv_addr.sin_port));
...
inline void rs232_TCPServer_Send(SerialPort *SerialPORT)
{
	if(ServerSocket.ReceiveFlag==-1)
	{
		...
		for(int i=0 ; i<MAX_CLIENT_NUMBER ; i++)
		{
			if(ServerSocket.Client_fd[i]>0)
			{
				if(SerialPORT->Aes.Enable==true) rs232_AES_Encrypt(SerialPORT);
				...
				else
				{
					...
					report("sdsd","RS232 -->  PORT ->>- TCP-Server:Socket",i," --  Count=",SerialPORT->RX_Count);
				}
	...
	else
	{
		...
		else
		{
			...
			report("sdsd","RS232 -->  PORT ->>- TCP-Server:Socket",ServerSocket.ReceiveFlag," --  Count=",SerialPORT->RX_Count);
		}
	}
...
inline void rs232_TCPClient_Send(SerialPort *SerialPORT)
{
		...
		for(int i=0 ; i<MAX_SERVER_NUMBER ; i++)
		{
			if(ClientSocket.Server_fd[i]>0 && ClientSocket.Status[i].Connected==true)
			{
				...
				else
				{
					...
					report("sdsd","RS232 -->  PORT ->>- TCP-Client:Socket",i," --  Count=",SerialPORT->RX_Count);
				}
				break;
			}
		}
	...
	else
	{
		...
		else
		{
			...
			report("sdsd","RS232 -->  PORT ->>- TCP-Client:Socket",uint8_t(ClientSocket.ReceiveFlag)," --  Count=",SerialPORT->RX_Count);
		}
	}
...
void *rs232_UDPServer_Receive(void *arg)
{
	...
	report("s","RS232 --> UDP_SERVER_RX_Thread Started !");
	...
	while(1)
	{
		...
		if(activity==-1)
		{
			 if(ClientSocket.ReceiveFlag >-1)
			 {
				 if(SerialPORT.TX_Count>0)
				 {
					 report("sdsd","RS232 -->    UDP-Server:Socket",ClientSocket.ReceiveFlag," ->>- PORT --  Count=",SerialPORT.TX_Count);
					 rs232_SerialPort_Send(&SerialPORT);
				 }
...
inline void rs232_UDPServer_Send(SerialPort *SerialPORT)
{
	if(ServerSocket.ReceiveFlag==-1)
	{
		...
		for(int i=0 ; i<MAX_CLIENT_NUMBER ; i++)
		{
			if(ServerSocket.Client_fd[i]>0)
			{
				...
				{
					report("sdsd","RS232 -->    PORT->>- UDP-Client:Socket",i," --  Count=",SerialPORT->RX_Count);
					break;
				}
	...
	else
	{
		send(ServerSocket.Client_fd[uint8_t(ServerSocket.ReceiveFlag)],SerialPORT->RX_Buffer,SerialPORT->RX_Count,0);
		 ...
		 report("sdsd","RS232 -->    PORT->>- UDP-Client:Socket",ServerSocket.ReceiveFlag," --  Count=",SerialPORT->RX_Count);
		 SerialPORT->RX_Count=0;
	}
}
...
void *rs232_UDPClient_Receive(void *arg)
{
	...
	report("s","RS232 --> UDP_CLIENT_RX_Thread Started !");
	...
	if(ClientSocket.ReceiveFlag==-1)
	{
		for(int i=0 ; i<MAX_SERVER_NUMBER ; i++)
		{
			if(ClientSocket.Server_fd[i]>0 )
			{
				...
				report("sdsd","RS232 -->    PORT -->> UDP-Server:Socket",ClientSocket.ReceiveFlag," --  Count=",SerialPORT->RX_Count);
			}
...
{
	char Buffer[RS232_BUFFER_SIZE];
	SerialPort &SerialPORT = *((SerialPort *)arg);
	uint16_t num_bytes;
	struct timeval  tim;

	report("s","RS232 --> PORT_RX_Thread Started !");
	...
	while(1)
	{
		...
		if(num_bytes>0)
		{
			...
			if(SerialPORT.RX_Count+num_bytes < RS232_BUFFER_SIZE)
			{
			...
			else
			{
				...
				{
					if(ServerSocket.Parameters.Type==TCP) rs232_TCPServer_Send(&SerialPORT);
					else rs232_UDPServer_Send(&SerialPORT);
				}
				else
				{
					if(ClientSocket.Parameters->Type==TCP ) rs232_TCPClient_Send(&SerialPORT);
					else rs232_UDPClient_Send(&SerialPORT);
				}
				exec2(LED_G232_SHOT);
				memcpy(SerialPORT.RX_Buffer+SerialPORT.RX_Count,Buffer,num_bytes);
				SerialPORT.RX_Count+=num_bytes;
		...
		else
		{
			...
			if(Dnp_Buffer.TX_Count>0)
			{
				...
				report("s","RS232 --> After DNP packet send!");
			}
